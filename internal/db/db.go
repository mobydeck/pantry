...
func (d *DB) InsertItem(item models.Item, details *string) (int64, error) {
	...
	relatedFilesJSON, _ := json.Marshal(item.RelatedFiles)
...
func (d *DB) InsertVector(rowid int64, embedding []float32) error {
	...
	// Convert []float32 to bytes for storage
...
func (d *DB) GetItem(itemID string) (*models.Item, bool, error) {
	...
	// Parse tags and related files
	json.Unmarshal([]byte(itemModel.Tags), &item.Tags)
	json.Unmarshal([]byte(itemModel.RelatedFiles), &item.RelatedFiles)
...
func (d *DB) VectorSearch(queryEmbedding []float32, limit int, project *string, source *string) ([]models.SearchResult, error) {
	...
	embeddingBytes, _ := json.Marshal(queryEmbedding)
...
func (d *DB) ListAllForReindex() ([]map[string]interface{}, error) {
	var itemModels []ItemModel
	if err := d.db.Order("rowid").Find(&itemModels).Error; err != nil {
		return nil, err
	}

	results := make([]map[string]interface{}, len(itemModels))
	for i, im := range itemModels {
		// Get rowid
		var rowid int64
